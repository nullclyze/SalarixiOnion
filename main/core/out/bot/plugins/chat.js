...
export class ChatController {
    ...
    async message(options) {
        if (!this.bot)
            return;
        try {
            this.object.updateTask('message', true, 0.3);
            let text = '';
            ...
            if (options.from === '@all') {
                ...
                if (this.object.nickname === options.from)
                    this.bot.chat(text);
            }
        }
        catch {
            this.object.updateTask('message', false);
        }
        finally {
            this.object.updateTask('message', false);
        }
    }
    async spamming(options) {
                ...
                while (this.tasks.spamming.status) {
                    ...
                    if (this.tasks.spamming.status) {
                        ...
                        if (options.useBypass) {
                            text += generator.chooseRandomValueFromArray([' : ', ' | ', ' / ']) + generator.generateRandomSpecialString(generator.generateRandomNumberBetween(3, 6));
                        }
                        if (valid) {
                            if (options.from === '@all') {
                                this.bot.chat(text);
