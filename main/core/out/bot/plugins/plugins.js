...
import { activeBots } from '../base.js';
...
async function sleep(synchronize, options) {
    if (synchronize) {
        if (!options.key)
            return;
...
export class Chat {
    async flow(bot, data) {
        switch (data.type) {
            case 'message':
                await this.message(bot, data.options);
                break;
            case 'spamming':
                await this.spamming(bot, data.options);
                break;
        }
    }
...
export class Action {
    constructor() {
        this.before = { pitch: 0, yaw: 0 };
    }
    async flow(bot, data) {
        switch (data.type) {
            case 'jumping':
                await this.jumping(bot, data.options);
                break;
            case 'shifting':
                await this.shifting(bot, data.options);
                break;
            case 'waving':
                await this.waving(bot, data.options);
                break;
            case 'spinning':
                await this.spinning(bot, data.options);
                break;
        }
    }
    async jumping(bot, options) {
        if (options.state === 'start') {
            if (!bot.object)
                return;
    ...
    async waving(bot, options) {
        if (options.state === 'start') {
            if (!bot.object)
                return;
            if (bot.tasks.waving.status)
                return;
            try {
    ...
    async spinning(bot, options) {
        if (options.state === 'start') {
            if (!bot.object)
                return;
            if (bot.tasks.spinning.status)
                return;
            try {
...
export class Imitation {
    ...
    async flow(bot, data) {
        switch (data.type) {
            case 'hybrid':
                await this.hybridImitation(bot, data.options);
                break;
            case 'walking':
                await this.walkingImitation(bot, data.options);
                break;
        }
    }
    ...
    async looking(bot, useSmoothness, useLongDelays) {
        if (!bot.object)
            return;
        if (bot.tasks.looking.status)
            return;
        try {
    ...
    async waving(bot, useLongDelays) {
        if (!bot.object)
            return;
        if (bot.tasks.waving.status)
            return;
    ...
    async hybridImitation(bot, options) {
        if (options.state === 'start') {
            if (!bot.object)
                return;
            if (bot.tasks.hybridImitation.status)
                return;
            if (bot.tasks.jumping.status || bot.tasks.shifting.status || bot.tasks.moveForward.status || bot.tasks.moveBack.status || bot.tasks.moveLeft.status || bot.tasks.moveRight.status)
                return;
            if (!options)
                return;
            try {
                ...
                if (options.useWaving)
                    this.waving(bot, options.useLongDelays);
                let isFirst = true;
                while (bot.tasks.hybridImitation.status) {
                    ...
                    for (const operation of operations) {
                        ...
                        for (const action of actions) {
                            if (!bot.tasks.hybridImitation.status || !bot.tasks.jumping.status || !bot.tasks.shifting.status || !bot.tasks.moveForward.status || !bot.tasks.moveBack.status || !bot.tasks.moveLeft.status || !bot.tasks.moveRight.status)
                                break;
    ...
    async walkingImitation(bot, options) {
        if (options.state === 'start') {
            if (!bot.object)
                return;
            if (bot.tasks.walkingImitation.status)
                return;
            if (bot.tasks.moveForward.status || bot.tasks.moveBack.status || bot.tasks.moveLeft.status || bot.tasks.moveRight.status || bot.tasks.looking.status)
                return;
            if (!options)
                return;
            try {
                ...
                while (bot.tasks.walkingImitation.status) {
                    ...
                    if (!bot.tasks.walkingImitation.status || !bot.tasks.moveForward.status || !bot.tasks.moveBack.status || !bot.tasks.moveLeft.status || !bot.tasks.moveRight.status)
                        break;
...
export class Flight {
    ...
    async flight(bot, type, options) {
        try {
            if (options.state === 'start') {
                ...
                switch (type) {
                    case 'default':
                        await this.defaultFlight(bot, options);
                        break;
                    case 'jump':
                        await this.jumpFlight(bot, options);
                        break;
                    case 'glitch':
                        await this.glitchFlight(bot, options);
                        break;
                }
            }
            else if (options.state === 'stop') {
    ...
    async defaultFlight(bot, options) {
        if (!bot.object)
            return;
    ...
    async glitchFlight(bot, options) {
        try {
            if (!bot.object)
                return;
            if (options.useSpoofing) {
...
export class Sprinter {
    ...
    async sprinter(bot, type, options) {
        try {
            if (options.state === 'start') {
                if (!options)
                    return;
                ...
                switch (type) {
                    case 'default':
                        await this.defaultSprinter(bot, options);
                        break;
                    case 'phantom':
                        await this.defaultSprinter(bot, options);
                        break;
    ...
    async defaultSprinter(bot, options) {
        if (!bot.object)
            return;
        if (options.useSpoofing) {
            bot.control('spoofer', 'on', { useSharpness: false, useBuffering: true });
...
export class Ghost {
    ...
    async clearPacketBuffer(bot) {
        if (!bot.object)
            return;
        if (this.packetBuffer[bot.nickname].size === 0)
            return;
    ...
    async temperatePacketIgnore(bot, options) {
        try {
            if (!bot.object)
                return;
    ...
    async normalPacketIgnore(bot, options) {
        try {
            if (!bot.object)
                return;
            if (!this.originalWrite[bot.nickname]) {
    ...
    async aggressivePacketIgnore(bot, options) {
        try {
            if (!bot.object)
                return;
            if (!this.originalWrite[bot.nickname]) {
