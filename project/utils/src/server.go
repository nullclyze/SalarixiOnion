...
func main() {
	http.HandleFunc("/salarixi/utils/config/write", writeConfigHandler)
	http.HandleFunc("/salarixi/utils/config/read", readConfigHandler)
...
func writeConfigHandler(w http.ResponseWriter, r *http.Request) {
	...
	status, message := tools.WriteConfig(body.Config)
...
func readConfigHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Access-Control-Allow-Origin", "*")
	w.Header().Set("Access-Control-Allow-Methods", "GET, POST, OPTIONS")
	w.Header().Set("Access-Control-Allow-Headers", "Content-Type")
	w.Header().Set("Content-Type", "application/json")
	...
	status, message, data := tools.ReadConfig()
