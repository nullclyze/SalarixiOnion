...
use crate::service::core::bot::set_mutual_states;
...
impl AntiAfkModule {
  ...
  async fn normal(&self, bot: &Client, options: &AntiAfkOptions) {
    ...
    loop {
      ...
      bot.set_direction(direction.0 + randfloat(-35.0, 35.0) as f32, direction.1 + randfloat(-20.0, 20.0) as f32);
    ...
    tokio::spawn(async move {
      loop {
        if let Some(bot) = BOT_REGISTRY.get_bot(&username) {
          ...
          bot.set_direction(direction.0 + randfloat(-35.0, 35.0) as f32, direction.1 + randfloat(-10.0, 10.0) as f32);
        }
