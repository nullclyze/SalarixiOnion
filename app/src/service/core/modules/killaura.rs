...
use crate::service::core::extensions::{entity_type_from, BotDefaultExt, BotInventoryExt, BotMovementExt, BotRotationExt};
...
impl KillauraModule {
  ...
  fn find_nearest_entity(bot: &Client, target: String, target_nickname: Option<String>, max_distance: f64) -> Option<Entity> {
    let target = if target == "custom" { target_nickname } else { Some(target) };
          ...
          match weapon.as_str() {
            "sword" => match item.kind() {
              ItemKind::WoodenSword => {
                weapons.push(Weapon { slot: Some(slot), priority: 0 });
              }
              ItemKind::GoldenSword => {
                weapons.push(Weapon { slot: Some(slot), priority: 1 });
              }
              ItemKind::StoneSword => {
                weapons.push(Weapon { slot: Some(slot), priority: 2 });
              }
              ItemKind::CopperSword => {
                weapons.push(Weapon { slot: Some(slot), priority: 3 });
              }
              ItemKind::IronSword => {
                weapons.push(Weapon { slot: Some(slot), priority: 4 });
              }
              ItemKind::DiamondSword => {
                weapons.push(Weapon { slot: Some(slot), priority: 5 });
              }
              ItemKind::NetheriteSword => {
                weapons.push(Weapon { slot: Some(slot), priority: 6 });
              }
              _ => {}
            },
            "axe" => match item.kind() {
  ...
  async fn moderate_killaura(&self, bot: &Client, options: &KillauraOptions) {
    ...
    loop {
      ...
      {
        if let Some(entity) = Self::find_nearest_entity(bot, options.target.clone(), options.target_nickname.clone(), config.attack_distance) {
          ...
          if options.use_critical {
            ...
            if let Some(e) = Self::find_nearest_entity(bot, options.target.clone(), options.target_nickname.clone(), config.attack_distance) {
              bot.attack(e);
            }
          } else {
            bot.attack(entity);
