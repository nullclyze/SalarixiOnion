...
use crate::service::core::bot::{get_state, kill_task, set_mutual_states};
use crate::service::core::common::get_block_state;
use crate::service::core::extensions::{BotDefaultExt, BotInventoryExt, BotRotationExt};
...
impl FarmerModule {
    ...
    if let Some(menu) = bot.get_inventory_menu() {
      for (slot, item) in menu.slots().iter().enumerate() {
        if !item.is_empty() {
          match item.kind() {
            ItemKind::WoodenHoe => {
              tools.push(Tool { slot: Some(slot), priority: 0 });
            }
            ItemKind::GoldenHoe => {
              tools.push(Tool { slot: Some(slot), priority: 1 });
            }
            ItemKind::StoneHoe => {
              tools.push(Tool { slot: Some(slot), priority: 2 });
            }
            ItemKind::CopperHoe => {
              tools.push(Tool { slot: Some(slot), priority: 3 });
            }
            ItemKind::IronHoe => {
              tools.push(Tool { slot: Some(slot), priority: 4 });
            }
            ItemKind::DiamondHoe => {
              tools.push(Tool { slot: Some(slot), priority: 5 });
            }
            ItemKind::NetheriteHoe => {
              tools.push(Tool { slot: Some(slot), priority: 6 });
            }
            _ => {}
          }
        }
  ...
  fn this_is_plant(&self, kind: BlockKind) -> bool {
    return vec![BlockKind::Potatoes, BlockKind::Carrots, BlockKind::Wheat, BlockKind::Beetroots].contains(&kind);
  }
  ...
  fn block_can_plowed(&self, bot: &Client, kind: BlockKind, block_pos: BlockPos) -> bool {
    if let Some(state) = get_block_state(bot, BlockPos::new(block_pos.x, block_pos.y + 1, block_pos.z)) {
      return vec![BlockKind::Dirt, BlockKind::DirtPath, BlockKind::RootedDirt, BlockKind::CoarseDirt].contains(&kind) && state.is_air();
    }
  ...
  async fn interact_with_block(&self, bot: &Client, block_pos: BlockPos, options: &FarmerOptions) {
    ...
    if !get_state(&nickname, "is_eating") && !get_state(&nickname, "is_drinking") && get_state(&nickname, "can_interacting") {
      if let Some(state) = get_block_state(bot, block_pos) {
