...
use thirtyfour::{
  common::capabilities::firefox::FirefoxPreferences, error::WebDriverError, CapabilitiesHelper, ChromiumLikeCapabilities, DesiredCapabilities, WebDriver,
};
use tokio::sync::broadcast;

use crate::api::emit::send_log;
use crate::service::core::footing::{current_options, CaptchaBypassOptions};
...
pub static MAP_ACCUMULATOR: Lazy<Arc<MapAccumulator>> = Lazy::new(|| Arc::new(MapAccumulator::new()));
...
impl WebCaptchaBypass {
  ...
  pub async fn create_webdriver(&self, browser: String, server_url: Option<String>, proxy: Option<thirtyfour::Proxy>) -> Result<WebDriver, WebDriverError> {
  ...
  pub async fn webdriver_event_loop(&'static self) {
    ...
    while let Ok(event) = rx.recv().await {
      match event {
        ...
        WebDriverEvent::CreateWebDriver { proxy, username, password } => {
          ...
          if let Some(options) = current_options() {
            ...
            match self
              .create_webdriver(options.captcha_bypass.browser, options.captcha_bypass.webdriver_server_url, manual_proxy)
              .await
            {
              Ok(driver) => {
  ...
  pub fn solve_captcha(&self, username: String, base64_code: String, options: CaptchaBypassOptions) {
    tokio::spawn(async move {
      ...
      if let Ok(ok_resp) = serde_json::from_slice::<Service2CaptchaOkResponse>(&body) {
        ...
        send_log(
          format!("Бот {} получил неизвестный формат ответа от {}: {}", username, options.api_service, body_str),
          "error",
        );
      }
    });
  ...
  pub fn combine_all(&self, username: &str) -> Option<String> {
    ...
    if !map_data.is_empty() {
      ...
      map_data.retain(|map| {
        ...
        let normalized_angle = if angle_diff > PI { 2.0 * PI - angle_diff } else { angle_diff };
