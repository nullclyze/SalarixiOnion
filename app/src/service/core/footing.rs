...
use crate::api::emit::*;
use crate::service::core::bot::*;
use crate::service::core::handlers::single::single_handler;
use crate::service::core::handlers::swarm::swarm_handler;
use crate::service::core::modules::*;
use crate::service::core::plugins::*;
use crate::service::core::registry::registry_event_loop;
...
use crate::service::generators::mutate_text;
use crate::service::generators::randchance;
use crate::service::generators::randelem;
use crate::service::generators::randstr;
use crate::service::generators::randuint;
use crate::service::generators::Classes;
use crate::service::webhook::send_webhook;

pub static ACTIVE: AtomicBool = AtomicBool::new(false);
pub static CURRENT_OPTIONS: Lazy<Arc<RwLock<Option<LaunchOptions>>>> = Lazy::new(|| Arc::new(RwLock::new(None)));
...
fn generate_username_or_password(item: &str, class: &str, template: &str) -> Option<String> {
  match class {
    "legit" => {
      let mut value = randelem(if item == "username" { LEGIT_NICKNAMES } else { LEGIT_PASSWORDS })
        .unwrap()
        .to_string();
      ...
      if value.len() <= 13 {
        let suffix_len = if value.len() <= 6 { randuint(4, 6) as usize } else { randuint(2, 3) as usize };
...
pub fn launch_bots_on_server(options: LaunchOptions) -> bool {
  if process_is_active() {
    send_log(format!("Запуск ботов невозможен: The process is already active"), "warning");
    return false;
  }
  ...
  if options.basic.use_anti_captcha && options.captcha_bypass.captcha_type.as_str() == "web" && options.captcha_bypass.solve_mode.as_str() == "auto" {
  ...
  if options.basic.use_webhook {
    send_webhook(options.webhook.url.clone(), format!("Запуск ботов на сервер {}...", options.basic.address));
  }

  if options.basic.use_webhook && options.webhook.send_data {
    send_webhook(options.webhook.url.clone(), format!("Опции запуска: {:#?}", options));
  }
      ...
      local_set.spawn_local(async move {
        ...
        if options.basic.use_accounts {
          ...
          for _ in 0..options.basic.bots_count {
            let Some(username) = generate_unique_username(&options.basic.nickname_type, &options.basic.nickname_template) else {
              continue;
            };

            let password = generate_username_or_password("password", &options.basic.password_type, &options.basic.password_template);
...
pub fn stop_bots_and_destroy_data() -> bool {
  ...
  if !process_is_active() {
    send_log(format!("Остановка ботов невозможна: The process is not active now"), "warning");
    return false;
  }
  ...
  if let Some(options) = current_options() {
    if options.basic.use_webhook {
      send_webhook(options.webhook.url.clone(), format!("{} ботов было остановлено", active_bots));
    }
  }
...
impl PluginManager {
  ...
  pub fn activate_for(&'static self, username: String, plugins: PluginOptions) -> io::Result<()> {
    self.tasks.write().unwrap().insert(username.clone(), HashMap::new());
