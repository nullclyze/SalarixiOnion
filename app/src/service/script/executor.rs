use azalea::{
  bot::BotClientExt, prelude::PathfinderClientExt, protocol::packets::game::s_interact::InteractionHand, BlockPos, SprintDirection, Vec3, WalkDirection,
};
...
use crate::service::core::common::convert_hotbar_slot_to_inventory_slot;
use crate::service::core::extensions::{
  entity_type_from, go_to, BotDefaultExt, BotInteractExt, BotInventoryExt, BotMovementExt, BotPhysicsExt, BotRotationExt, InvClick,
};
use crate::service::core::{
  bot::{get_state, set_state, PROFILES},
  footing::{active_bots_count, current_options},
  ...
  pub fn execute(&self, su: String, script: String) {
    ...
    engine
      .register_fn("get_bot_id", {
        let su = su.clone();
        move |mu: &str| BOT_REGISTRY.get_bot(&Self::parse_username(&su, mu)).map(|b| b.id().0 as i64).unwrap_or(0)
      })
      .register_fn("get_bot_ping", {
        let su = su.clone();
        move |mu: &str| BOT_REGISTRY.get_bot(&Self::parse_username(&su, mu)).map(|b| b.ping() as i64).unwrap_or(0)
      })
      ...
      .register_fn("bot_is_workable", {
        let su = su.clone();
        move |mu: &str| BOT_REGISTRY.get_bot(&Self::parse_username(&su, mu)).map(|b| b.workable()).unwrap_or(false)
      })
      .register_fn("get_bot_profile", {
        let su = su.clone();
        move |mu: &str| Self::create_dynamic_profile(&Self::parse_username(&su, mu))
      });

    engine
      .register_fn("chat", {
        let su = su.clone();
        move |mu: &str, message: &str| Self::chat(&Self::parse_username(&su, mu), message.to_string())
      })
      .register_fn("jump", {
        let su = su.clone();
      ...
      .register_fn("set_velocity", {
        let su = su.clone();
        move |mu: &str, axis: &str, velocity: f64| Self::set_velocity(&Self::parse_username(&su, mu), axis, velocity)
      })
      .register_fn("set_on_ground", {
        let su = su.clone();
        move |mu: &str, on_ground: bool| Self::set_on_ground(&Self::parse_username(&su, mu), on_ground)
      })
      .register_fn("set_old_position", {
        let su = su.clone();
        move |mu: &str, x: f64, y: f64, z: f64| Self::set_old_position(&Self::parse_username(&su, mu), x, y, z)
      })
      .register_fn("set_no_jump_delay", {
        let su = su.clone();
        move |mu: &str, delay: i64| Self::set_no_jump_delay(&Self::parse_username(&su, mu), delay as u32)
      })
      .register_fn("set_was_touching_water", {
        let su = su.clone();
        move |mu: &str, state: bool| Self::set_was_touching_water(&Self::parse_username(&su, mu), state)
      })
      ...
      .register_fn("start_walking", {
        let su = su.clone();
        move |mu: &str, direction: &str| Self::start_walking(&Self::parse_username(&su, mu), direction.to_string())
      })
      .register_fn("start_sprinting", {
        let su = su.clone();
        move |mu: &str, direction: &str| Self::start_sprinting(&Self::parse_username(&su, mu), direction)
      })
      .register_fn("start_pathfinder", {
        let su = su.clone();
        move |mu: &str, x: i64, z: i64| Self::start_pathfinder(&Self::parse_username(&su, mu), x as i32, z as i32)
      })
    ...
    engine
      .register_fn("place_block", {
        let su = su.clone();
        move |mu: &str, x: i64, y: i64, z: i64| Self::place_block(&Self::parse_username(&su, mu), x as i32, y as i32, z as i32)
      })
      .register_fn("look_at", {
        let su = su.clone();
        move |mu: &str, x: f64, y: f64, z: f64| Self::look_at(&Self::parse_username(&su, mu), x, y, z)
      })
      .register_fn("look_at_block", {
        let su = su.clone();
        move |mu: &str, x: i64, y: i64, z: i64| Self::look_at_block(&Self::parse_username(&su, mu), x as i32, y as i32, z as i32)
      });
    ...
    engine
      ...
      .register_fn("inv_click", {
        let su = su.clone();
        move |mu: &str, slot: i64, mode: i64, lock: bool| Self::inv_click(&Self::parse_username(&su, mu), slot as usize, mode as u8, lock)
      })
      .register_fn("inv_click_on", {
        let su = su.clone();
        move |mu: &str, name: &str, mode: i64, lock: bool| Self::inv_click_on(&Self::parse_username(&su, mu), name, mode as u8, lock)
      })
      .register_fn("set_selected_slot", {
        let su = su.clone();
        move |mu: &str, slot: i64| Self::set_selected_slot(&Self::parse_username(&su, mu), slot as u8)
      });
    ...
    engine
      .register_fn("set_state", {
        let su = su.clone();
        move |mu: &str, field: &str, value: bool| set_state(&Self::parse_username(&su, mu), field, value)
      })
